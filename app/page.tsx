...
export default function HomePage() {
  ...
  return <div className="page-frame swiss-home">
    ...
    <section className="section home-ask" aria-labelledby="ask-heading"><SectionHeading id="ask-heading" eyebrow={{ zh: "向檔案提問 / 02", en: "Archive Q&A / 02" }} title={{ zh: "如果好奇，就直接問。", en: "Ask what you are curious about." }} description={{ zh: "目前的回答取自公開檔案，也會帶你回到相關章節。問答仍是前端示範，引用的內容則來自真實資料。", en: "Answers come from public records and point back to relevant sections. The chat is still a front-end demo, while its citations use real archive material." }} locale={locale} /><AskInterface /></section>

    <section className="section topic-section" aria-labelledby="topics-heading"><SectionHeading id="topics-heading" eyebrow={{ zh: "主題索引 / 03", en: "Topic index / 03" }} title={{ zh: "幾條理解專案的線索。", en: "A few ways into the projects." }} description={{ zh: "架構、控制、感知與安全不是能力標籤，而是串起論文與實驗紀錄的主題。", en: "Architecture, control, perception, and safety are themes connecting the thesis and experiment records, not capability scores." }} locale={locale} /><div className="topic-index">{topicIndex.map((topic, index) => <span className="topic-item" key={topic.zh}><b>{String(index + 1).padStart(2, "0")}</b>{t(topic, locale)}</span>)}</div></section>
  </div>;
}
