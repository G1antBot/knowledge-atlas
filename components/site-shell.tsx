...
function ShellContent({ children }: { children: React.ReactNode }) {
  ...
  return <div className="site-root">
    <header className="site-header">
      ...
      <nav id="main-navigation" className={`main-nav ${menuOpen ? "is-open" : ""}`} aria-label={locale === "zh" ? "主導覽" : "Main navigation"}>
      ...
      <div className="header-actions">
        ...
        <button className="locale-switch" type="button" onClick={toggleLocale} aria-label={locale === "zh" ? "切換為英文" : "Switch to Chinese"}><span className={locale === "zh" ? "selected" : ""}>中</span><span>/</span><span className={locale === "en" ? "selected" : ""}>EN</span></button>
        <button className={`menu-toggle ${menuOpen ? "is-open" : ""}`} type="button" onClick={() => setMenuOpen(!menuOpen)} aria-expanded={menuOpen} aria-controls="main-navigation" aria-label={menuLabel}><span /><span /><span /><b>{locale === "zh" ? "選單" : "Menu"}</b></button>
      </div>
    </header>
    <main>{children}</main>
