  ...
  return <div className="page-frame project-editorial">
    ...
    {isPrimary ? <>
      ...
      <article className="editorial-article">{project.sections.map((section, index) => <ArchiveSectionView key={section.id} section={section} index={index} />)}
        {project.media && <section className="editorial-section editorial-media" id="media"><div className="editorial-section-index"><span>{String(project.sections.length + 1).padStart(2, "0")}</span><small>media / record</small></div><div className="editorial-section-copy"><h2>{zh ? "媒体记录" : "Media record"}</h2><p>{zh ? "四段软件在环演示默认显示轻量 poster；点击后才加载对应 GIF。" : "Four software-in-the-loop demonstrations use lightweight posters and load their GIF only on demand."}</p></div><div className="editorial-section-figures"><MediaStrip assets={project.media} /></div></section>}
        <section className="project-source-ledger"><div><Eyebrow>{zh ? "来源台账" : "Source ledger"}</Eyebrow><h2>{zh ? "每条结论回到它的出处。" : "Every claim returns to its source."}</h2></div><SourceList sources={project.sources} /></section>
        <ArchiveRelations project={project} projects={archiveProjects} />
        <Link className="related-link" href="/projects"><span>{zh ? "返回档案索引" : "Back to archive index"}</span><Arrow /></Link>
      </article>
    </> : <div className="pending-archive"><Eyebrow tone={project.accent === "ink" ? "ink" : project.accent}>{project.status === "pending" ? (zh ? "待补档" : "Archive pending") : (zh ? "次级档案" : "Secondary archive")}</Eyebrow><h2>{t(project.subtitle, locale)}</h2><p>{t(project.summary, locale)}</p><SourceList sources={project.sources} /><Link className="related-link" href="/projects"><span>{zh ? "返回档案索引" : "Back to archive index"}</span><Arrow /></Link></div>}
  </div>;
}
