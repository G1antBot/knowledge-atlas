...
function MediaStage({ asset }: { asset: MediaAsset }) {
  ...
  const zh = locale === "zh";
  const actionLabel = active ? (zh ? `暂停${t(asset.title, locale)}演示` : `Pause ${t(asset.title, locale)} demo`) : (zh ? `播放${t(asset.title, locale)}演示` : `Play ${t(asset.title, locale)} demo`);
  return <figure className={`media-stage ${videoReady ? "has-video" : ""}`} data-media-mode={active ? (asset.videoPath ? "video" : "animated-gif") : "static-poster"}>
    <div className="media-visual">
      ...
      <button className="media-play" type="button" onClick={() => { setActive((current) => !current); setVideoReady(false); }} aria-pressed={active} aria-label={actionLabel}><span aria-hidden="true">{active ? "Ⅱ" : "▶"}</span><b>{active ? (zh ? "静态" : "Poster") : (zh ? "播放" : "Play")}</b></button>
...
export function MediaStrip({ assets }: { assets: MediaAsset[] }) {
  const { locale } = useLocale();
  return <div className="media-strip" aria-label={locale === "zh" ? "无人机实验媒体条" : "UAV experiment media strip"}>{assets.map((asset) => <MediaStage asset={asset} key={asset.id} />)}</div>;
}
